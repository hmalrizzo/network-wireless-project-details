...
for b_ = 1:length(eNodeBs)
    ...
    %Testing to draw a custom network - Wajahat Baig 09/11/2010
    
    
end

%% Cutting off power for selected eNodeBs - Wajahat Baig 02/09/2010
% switch LTE_config.switch_off_eNodeBs
%     case 1
%         for b_ = 1:length(eNodeBs)
%             for s_ = 1:number_of_sectors
%                 if(b_== 1 || b_== 3 || b_== 4 || b_== 6)
%                     eNodeBs(b_).sectors(s_).max_power = 0;
%                 end
%             end
%         end
%     case 2
...
%         end
%     case 4
%             eNodeBs(1).sectors(1).max_power = 0;
%             eNodeBs(1).sectors(2).max_power = 0;
%             eNodeBs(1).sectors(3).max_power = 0;
%             
%             eNodeBs(2).sectors(1).max_power = eNodeB_sector_tx_power;
%             eNodeBs(2).sectors(2).max_power = 0;
%             eNodeBs(2).sectors(3).max_power = 0;
%             
%             eNodeBs(3).sectors(1).max_power = 0;
%             eNodeBs(3).sectors(2).max_power = eNodeB_sector_tx_power;
%             eNodeBs(3).sectors(3).max_power = 0;
%             
%             eNodeBs(4).sectors(1).max_power = 0;
%             eNodeBs(4).sectors(2).max_power = 0;
%             eNodeBs(4).sectors(3).max_power = 0;
%             
%             eNodeBs(5).sectors(1).max_power = eNodeB_sector_tx_power;
%             eNodeBs(5).sectors(2).max_power = 0;
%             eNodeBs(5).sectors(3).max_power = 0;
%             
%             eNodeBs(6).sectors(1).max_power = 0;
%             eNodeBs(6).sectors(2).max_power = eNodeB_sector_tx_power;
%             eNodeBs(6).sectors(3).max_power = 0;
%             
%             eNodeBs(7).sectors(1).max_power = eNodeB_sector_tx_power;
%             eNodeBs(7).sectors(2).max_power = 0;
%             eNodeBs(7).sectors(3).max_power = 0;
%     case 5
%         eNodeBs(1).sectors(1).max_power = eNodeB_sector_tx_power;
%         eNodeBs(1).sectors(2).max_power = 0;
%         eNodeBs(1).sectors(3).max_power = 0;
%         
%         eNodeBs(2).sectors(1).max_power = eNodeB_sector_tx_power;
...
%         eNodeBs(2).sectors(3).max_power = 0;
%             
%         eNodeBs(3).sectors(1).max_power = eNodeB_sector_tx_power;
%         eNodeBs(3).sectors(2).max_power = 0;
%         eNodeBs(3).sectors(3).max_power = 0;
%     case 6
%         eNodeBs(1).sectors(1).max_power = eNodeB_sector_tx_power;
%         eNodeBs(1).sectors(2).max_power = eNodeB_sector_tx_power;
%         eNodeBs(1).sectors(3).max_power = eNodeB_sector_tx_power;
%         
%         eNodeBs(2).sectors(1).max_power = eNodeB_sector_tx_power;
%         eNodeBs(2).sectors(2).max_power = eNodeB_sector_tx_power;
%         eNodeBs(2).sectors(3).max_power = eNodeB_sector_tx_power;
%             
%         eNodeBs(3).sectors(1).max_power = eNodeB_sector_tx_power;
%         eNodeBs(3).sectors(2).max_power = eNodeB_sector_tx_power;
%         eNodeBs(3).sectors(3).max_power = eNodeB_sector_tx_power;
%     
%     otherwise
%         
% end
...
switch LTE_config.switch_off_eNodeBs
    ...
    case 4
        ...
        eNodeBs(4).sectors(1).max_power = 0;
        eNodeBs(4).sectors(2).max_power = eNodeB_sector_tx_power;
        eNodeBs(4).sectors(3).max_power = 0;
             
        eNodeBs(5).sectors(1).max_power = 0;
        eNodeBs(5).sectors(2).max_power = eNodeB_sector_tx_power;
        eNodeBs(5).sectors(3).max_power = 0;        
    otherwise
...
% Assigned independant gains - Wajahat Baig 02/09/2010
    angle = -180:0.1:180;
    gain = zeros(1,length(angle));
    for i_=1:length(angle)
        for e_ = 1:length(eNodeBs)
            for s_ = 1:number_of_sectors
                gain(i_) = eNodeBs(e_).sectors(s_).antenna.gain(angle(i_));
            end
        end
    end
end
