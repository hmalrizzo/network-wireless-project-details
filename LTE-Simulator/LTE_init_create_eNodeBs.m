...
switch LTE_config.eNodeB_arrangment
    case 0
    ...
    case 1
        temp = -1000;
        for b_ = 1:3
            temp = temp + inter_bts_distance;
            eNodeBs(b_) = network_elements.eNodeB;
            eNodeBs(b_).id = b_;
            eNodeBs(b_).pos = [temp 0];
        end
    ...
    case 2
        ...
        eNodeBs(4) = network_elements.eNodeB;
        eNodeBs(4).id = 4;
        eNodeBs(4).pos = [-250 500];
        
        eNodeBs(5) = network_elements.eNodeB;
        eNodeBs(5).id = 5;
        eNodeBs(5).pos = [250 500];
...
%% Calculate which are the neighboring eNodeBs
