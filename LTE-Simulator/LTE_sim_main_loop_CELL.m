...
while networkClock.current_TTI < LTE_config.simulation_time_tti
    ...
    for u_ = 1:length(UEs)
        ...
        check_x = [check_x;x_range];
        check_y = [check_y;y_range];
        ...
        if ~UEs(u_).is_in_roi(x_range,y_range)
            ...
            if (networkClock.current_TTI > 1)
                ...
                [new_eNodeB_id new_eNodeB_sector] = networkPathlossMap.cell_assignment(UEs(u_).pos);
    ...
    % Calculating User Satisfaction Level at this instant - Wajahat Baig
    % 18/12/2010
    
    for u_ = 1:length(UEs)
        s1 = 1;
        s2 = 1;
        f1 = simulation_traces.UE_traces(u_).avg_throughput(1,networkClock.current_TTI);
        f2 = simulation_traces.UE_traces(u_).avg_throughput(2,networkClock.current_TTI);
        delta = LTE_config.TTI_length;
        
        N = 0;
        N = UEs(u_).attached_eNodeB.sectors(UEs(u_).attached_sector).attached_UEs;
        %N = length(UEs);
        n = length(UEs);
        T = LTE_config.simulation_time_tti * delta;
        d_max = LTE_config.feedback_channel_delay;
        satisfaction = (((s1*f1)+(s2*f2))*T*ceil(N/n)*delta)/((T+d_max)*s1);
        UEs(u_).satisfaction= [UEs(u_).satisfaction;satisfaction ];
    end
