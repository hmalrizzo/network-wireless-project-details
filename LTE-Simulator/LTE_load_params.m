...
%% Plotting options
LTE_config.show_network = 2; % 0= show no plots
                             % 1= show some plots
...
LTE_config.target_sector = 'center';%[5 1];%'center';          % 'auto' for specifying the target sector to be the center one
...
LTE_config.nr_UEs = 5;    % number of users per eNodeB sector
LTE_config.UE_speed      = 15/3.6; % Speed at which the UEs move. In meters/second: 5 Km/h = 1.38 m/s
...
%LTE_config.UE_cache_file = fullfile('./data_files/UE_cache_1rings_2UEs_sector_20100625_093136.mat');

LTE_config.UE_cache_file           = fullfile('./data_files',...
        sprintf('UE_cache_%drings_%dUEs.mat',...
...
LTE_config.pregenerated_ff_file           = fullfile('./data_files',...
        sprintf('ff_%3.1fs_%dx%d_%s_%3.1fMHz_%dKmph.mat',...
