classdef lteScheduler < handle
    ...
    methods
        ...
        function [assigned_RBs CQIs_to_average_all UE_scheduled] = filter_out_zero_RBs_and_get_CQIs(obj,RB_grid,nCodewords,UE_CQI_feedback,current_UE)
            % Do not use RBs with a CQI of 0 (they are lost)
            if nCodewords == 1
                RB_grid.user_allocation((RB_grid.user_allocation==current_UE.id)&(UE_CQI_feedback'<1)) = 0;
                assigned_RBs = (RB_grid.user_allocation==current_UE.id);
                ...
                RB_grid.user_allocation((RB_grid.user_allocation==current_UE.id)&(zero_CQIs>=1)) = 0;
                assigned_RBs = (RB_grid.user_allocation==current_UE.id);
                CQIs_to_average_all = UE_CQI_feedback(assigned_RBs,:);
            end
